...
namespace COMMANDS
{
    public class CMD_DatabaseExtension_GraphicPanels : CMD_DatabaseExtension 
    {
        ...
        private static IEnumerator SetLayerMedia(string[] data)
        {
            ...
            if (graphic == null)
            {
                Debug.LogError($"Could not find media flie called '{mediaName}' in the Resources directories. Please specify the full path within resources and make sure that the file exists!");
                yield break;
            }
