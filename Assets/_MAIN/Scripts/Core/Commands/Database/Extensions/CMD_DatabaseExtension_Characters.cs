...
namespace COMMANDS
{
    public class CMD_DatabaseExtension_Characters : CMD_DatabaseExtension 
    {
        ...
        {
            database.AddCommand("createcharacter", new Action<string[]>(CreateCharacters));
        ...
        private static IEnumerator Show(string[] data)
        {
            ...
            {
                //A long running process should have a stop action to cancel out the coroutine and run logic that should complete this commnad
        ...
        {
            int employeeCountChange = 0;
            int employeeHappinessChange = 0;
            float employeeSkillChange = 0;
            int marketingChange = 0;
            int reputationChange = 0;
            int budgetChange = 0;
            int workChange = 0;
            float workingChange = 0;

            // Grab the extra parameters
            var parameters = ConvertDataToParameters(data, startingIndex: 1);

            // Try to get the values
            parameters.TryGetValue(new string[] { "-ec", "-employeecount" }, out employeeCountChange);
            parameters.TryGetValue(new string[] { "-eh", "-employeehappiness" }, out employeeHappinessChange);
            parameters.TryGetValue(new string[] { "-es", "-employeeskill" }, out employeeSkillChange);
            parameters.TryGetValue(new string[] { "-m", "-marketing" }, out marketingChange);
            parameters.TryGetValue(new string[] { "-r", "-reputation" }, out reputationChange);
            parameters.TryGetValue(new string[] { "-b", "-budget" }, out budgetChange);
            parameters.TryGetValue(new string[] { "-w", "-work" }, out workChange);
            parameters.TryGetValue(new string[] { "-wk", "-working" }, out workingChange);

            // Send the values to the DecisionManager to change the status of the company
            DecisionManager.instance.MakeDecision(
                employeeCountChange,
                employeeHappinessChange,
                employeeSkillChange,
                marketingChange,
                reputationChange,
                budgetChange,
